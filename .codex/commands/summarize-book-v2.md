...
This command is a variant of `/summarize-book`. It differs in exactly three ways:

1. **Voice:** neutral direct-statement, not third-person author attribution and not first person.
2. **No counterpoints.**
3. **No comprehension quiz.**

The output filename is the same as `/summarize-book` — `<book>_summary.html` — so running this command on a book that already has a standard summary will **overwrite** it. Always check for an existing file and ask the user before overwriting.
...
**IMPORTANT — prefer fidelity over tool-count minimization.** The goal is to detect accurate chapter boundaries and read every summarized section in a way that preserves source detail. Do not assume that a large Bash dump means the full book is now safely in working context; context quality matters more than raw tool payload size.
...
3. Measure the extracted text file you will use for summarization (`<book-name>_text.txt` or `clean.txt` if present). Record both:
   - Total line count
   - Total byte size

4. Choose one of these two ingestion paths:
   - **Small-book path:** Allowed only if the text is `<= 4,000` lines **and** `<= 250 KB`. In this case, you may read the full book in one pass and summarize all chapters in one shot.
   - **Large-book path:** Mandatory if the text exceeds either threshold. In this case, you must summarize in contiguous chapter batches. Do **not** attempt a one-shot whole-book summary.

5. **Small-book path details**
   - Read the full text with one or two Bash calls.
   - Confirm that every chapter range you intend to summarize was actually read.
   - If you cannot verify that all chapter ranges were actually read, stop and report the limitation instead of summarizing.

6. **Large-book path details**
   - Group chapters into contiguous batches capped at `<= 1,200` lines or `<= 90 KB` raw text per batch, whichever limit is hit first.
   - Never split a chapter merely for convenience. Only split a chapter if that single chapter alone exceeds one of the batch limits.
   - If a single chapter exceeds the cap, split it into ordered subranges, summarize each subrange into concise notes first, then synthesize those notes into the final chapter summary.
   - After finishing each batch, keep only short continuity notes from earlier chapters in working memory. Do not carry raw text from prior batches forward unless needed for a direct continuity reference.
   - Continue until every chapter range has been read and summarized.

7. **Codex-specific rule:** The default path must work without subagents. Do not rely on delegation for this workflow. You may mention optional delegation only if the user explicitly asks for delegation or parallel agent work.

8. Tell the user what chapters you found before proceeding. For example:
   "Found 18 chapters: Introduction (lines 1-1136), Chapter 1: Title (lines 1145-2185), ..."

9. **Hard stop rule:** If all chapter ranges were not actually read, do not summarize the book. Stop and report that limitation explicitly.

---

## Step 4: Summarize All Chapters — Neutral Voice

Summarize every chapter only after its full range has actually been read via the path chosen in Step 3. For small books, this may be a single-pass summary. For large books, this will be a batch-based process that still produces one final coherent summary file.

For each chapter, write explanatory paragraphs obeying these rules:

- Default to **3–4 paragraphs** for a normal chapter.
- Use **4–5 paragraphs** for long or concept-dense chapters.
- Use **2 paragraphs only for genuinely short sections** such as a brief postscript or similarly short concluding material.
- For each substantive chapter, include at least **two concrete anchors** drawn from the source text: a number, named concept, formal rule, case study, example company, or similarly specific detail.
...
- Reference ideas from prior chapters for continuity, but in neutral voice and with specific callbacks when relevant (`"Building on the 25–75 stock allocation rule introduced in Chapter 4..."`, not `"building on earlier ideas"` and not `"as the author showed in Chapter 5..."`).
...
- **Do NOT generate a quiz.** Skip Step 5 of the standard command entirely.
...
# 4. No harness metadata leaked into the output
...
# 6. For books with >= 18 sections, the output should usually be at least 6,000 words
python3 - <<'PY'
from pathlib import Path
import re
text = Path("summaries/<output-filename>").read_text()
body = re.sub(r"<[^>]+>", " ", text)
words = len(re.findall(r"\b\w+[\w'-]*\b", body))
print(words)
PY
# Expected: >= 6000 unless the source book is unusually short

# 7. Average paragraph count should usually be >= 3 per section
python3 - <<'PY'
from pathlib import Path
import re
text = Path("summaries/<output-filename>").read_text()
sections = text.count("<h2>")
paras = text.count("<p>")
print(paras / sections if sections else 0)
PY
# Expected: >= 3 for normal full-length books
```

Then manually spot-check at least three sections across the book — one early, one middle, one late — and confirm that each includes specific source-grounded details rather than generic thematic paraphrase. For example, verify the presence of named concepts or examples such as `25–75` stock allocation, `Mr. Market`, `net-current-asset` bargains, `special situations`, or `margin of safety` when summarizing books that contain them.

If any of the first four checks return non-zero, fix the file before reporting completion. If checks 6 or 7 fail for a normal full-length book, treat that as a quality failure and revise the summary before reporting completion.
