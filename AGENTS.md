...
- **`.claude/commands/`** — Claude Code workflow specifications. Leave these unchanged so they continue to work inside Claude Code.
- **`.codex/commands/`** — Codex-native mirrors of the Claude workflows. Codex uses only these files at runtime.
...
- In Codex, these workflow names are repository commands, not native Codex slash commands.
- Resolve Codex workflow requests only from **`.codex/commands/`**.
- Map the user-supplied argument string to `$ARGUMENTS` exactly as the Codex command file expects.
- Do not execute `.claude/commands/*` from Codex.
- Do not invoke `claude -p` from Codex.
- If a requested workflow does not exist in `.codex/commands/`, stop and tell the user that the workflow has not been migrated to Codex yet.
